...
class AutoCapture {
    ...
    captureClick(e) {
        ...
        if (!target)
            return;
        if (target.closest('[data-nohmo-ignore]'))
            return;
        ...
        if (existing && now - existing.ts < 1000) {
            ...
            existing.ts = now;
            if (existing.count === 3) {
                this.tracker.send('RAGE_CLICK', Object.assign(Object.assign({}, props), { x: e.clientX, y: e.clientY }));
            }
        }
        else {
            this.clickCounts.set(key, { count: 1, ts: now });
        }
        this.tracker.send('CLICK', Object.assign(Object.assign({}, props), { x: e.clientX, y: e.clientY }));
    }
    captureSubmit(e) {
        const form = e.target;
        if (!form)
            return;
        if (form.closest('[data-nohmo-ignore]'))
            return;
    ...
    captureInput(e) {
        ...
        if (!input)
            return;
        if (input.closest('[data-nohmo-ignore]'))
            return;
        if (input.type === 'password' ||
            input.type === 'hidden' ||
            input.hasAttribute('data-sensitive') ||
            ((_a = input.autocomplete) === null || _a === void 0 ? void 0 : _a.includes('cc')) ||
            ((_b = input.autocomplete) === null || _b === void 0 ? void 0 : _b.includes('password')))
            return;
    ...
    getRelevantElement(target) {
        let el = target;
        for (let i = 0; i < 5; i++) {
            if (!el)
                return target;
            if (RELEVANT_TAGS.has(el.tagName))
                return el;
            if (el.dataset.track || el.dataset.nohmo)
                return el;
            el = el.parentElement;
        }
        return target;
...
class EventQueue {
    constructor(flushFn, interval = 3000) {
        this.queue = [];
        this.timer = null;
...
class NohmoTracker {
    constructor(config) {
        ...
        this.config = Object.assign({ flushInterval: 3000, debug: false, autoPageView: true, autoScrollDepth: true, autoTimeSpent: true, autoCapture: true }, config);
        this.state = {
            deviceId: null,
            userId: null,
        ...
        this.queue = new EventQueue((events) => this.sendBatch(events), this.config.flushInterval);
    }
    async init() {
        var _a;
        if (typeof window === 'undefined')
        ...
        try {
            ...
            const res = await fetch(`${this.config.apiUrl}/api/tracker/identify/`, {
                method: 'POST',
                headers: {
            ...
            this.state.userId = (_a = data.userId) !== null && _a !== void 0 ? _a : null;
    ...
    async linkUser(userId, email, meta) {
    ...
    trackPageView(path) {
        this.send('PAGE_VIEW', {
            path: path !== null && path !== void 0 ? path : (typeof window !== 'undefined' ? window.location.pathname : ''),
    ...
    startScrollTracking() {
        ...
        const onScroll = () => {
            ...
            const milestone = [25, 50, 75, 100].find((m) => depth >= m && maxDepth < m);
    ...
    destroy() {
        var _a;
        (_a = this.autoCapture) === null || _a === void 0 ? void 0 : _a.stop();
        this.queue.destroy();
    }
}

const NohmoContext = react.createContext({
    tracker: null,
    send: () => undefined,
...
function NohmoProvider({ children, projectId, apiKey, apiUrl, options = {}, }) {
    ...
    react.useEffect(() => {
        const tracker = new NohmoTracker(Object.assign({ projectId,
            apiKey,
            apiUrl }, options));
        ...
        return () => {
            cleanupScroll === null || cleanupScroll === void 0 ? void 0 : cleanupScroll();
            tracker.destroy();
        };
    ...
    return (jsxRuntime.jsx(NohmoContext.Provider, { value: { tracker: trackerRef.current, send, linkUser }, children: children }));
}
function useNohmo() {
    return react.useContext(NohmoContext);
}
...
function NohmoNextProvider({ children, projectId, apiKey, apiUrl, options = {}, }) {
    return (jsxRuntime.jsxs(NohmoProvider, { projectId: projectId, apiKey: apiKey, apiUrl: apiUrl, options: options, children: [jsxRuntime.jsx(NohmoNextInner, {}), children] }));
}

exports.AutoCapture = AutoCapture;
exports.NohmoNextProvider = NohmoNextProvider;
exports.NohmoProvider = NohmoProvider;
exports.NohmoTracker = NohmoTracker;
exports.useNohmo = useNohmo;
exports.useNohmoNext = useNohmo;
